...
func TestRunner_Inspect_AllPending(t *testing.T) {
	...
	if len(status) != 4 {
		t.Fatalf("expected 4 statuses, got %d", len(status))
	}
...
func TestRunner_PendingCount_Some(t *testing.T) {
	...
	// 000001 is already applied, 000002, 000003, and 000004 are pending
	if count != 3 {
...
func TestRunner_DryRun_Up_Pending(t *testing.T) {
	...
	if len(names) != 4 {
		t.Errorf("expected 4 migrations, got %d", len(names))
	}
...
func TestRunner_UpgradeTo_AlreadyApplied(t *testing.T) {
	...
	if len(result.Skipped) != 4 {
		t.Errorf("expected 4 skipped, got %d", len(result.Skipped))
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("unmet expectations: %v", err)
	}
...
func TestRunner_PendingCount_NoMigrations(t *testing.T) {
	...
	// DirectionDown has 4 files (000001, 000002, 000003, 000004).
...
func TestRunner_Summary_HasValues(t *testing.T) {
	...
	if s["pending"] != 3 {
		t.Errorf("expected pending=3, got %d", s["pending"])
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("unmet expectations: %v", err)
	}
