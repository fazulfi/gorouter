...
func TestRunner_Migrate_Up_SkipsApplied(t *testing.T) {
	...
	if len(result.Applied) != 4 {
		t.Errorf("expected 4 applied, got %d", len(result.Applied))
	}
...
func TestRunner_ListPending_NonePending(t *testing.T) {
	...
	if len(pending) != 4 {
		t.Fatalf("expected 4 pending, got %d", len(pending))
	}
...
func TestRunner_Inspect_AllPending(t *testing.T) {
	...
	if len(status) != 5 {
		t.Fatalf("expected 5 statuses, got %d", len(status))
	}
...
func TestRunner_PendingCount_Some(t *testing.T) {
	...
	// 000001 is already applied, 000002-000005 are pending
	if count != 4 {
		t.Errorf("expected 4 pending, got %d", count)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("unmet expectations: %v", err)
	}
...
func TestRunner_DryRun_Up_Pending(t *testing.T) {
	...
	if len(names) != 5 {
		t.Errorf("expected 5 migrations, got %d", len(names))
	}
...
func TestRunner_DryRun_NonePending(t *testing.T) {
	...
	if len(names) != 4 {
		t.Fatalf("expected 4 migrations, got %d", len(names))
	}
...
func TestRunner_UpgradeTo_AlreadyApplied(t *testing.T) {
	...
	if len(result.Skipped) != 5 {
		t.Errorf("expected 5 skipped, got %d", len(result.Skipped))
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("unmet expectations: %v", err)
	}
...
func TestRunner_PendingCount_NoMigrations(t *testing.T) {
	...
	// DirectionDown has 5 files (000001-000005).
...
func TestRunner_Summary_HasValues(t *testing.T) {
	...
	if s["pending"] != 4 {
		t.Errorf("expected pending=4, got %d", s["pending"])
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("unmet expectations: %v", err)
	}
