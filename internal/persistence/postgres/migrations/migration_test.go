...
func TestRunner_Migrate_Up_AppliesAll(t *testing.T) {
	...
	if len(result.Applied) != 6 {
		t.Errorf("expected 6 applied, got %d", len(result.Applied))
	}
...
func TestRunner_Migrate_Up_SkipsApplied(t *testing.T) {
	...
	if len(result.Applied) != 5 {
		t.Errorf("expected 5 applied, got %d", len(result.Applied))
	}
...
func TestRunner_ListPending_AllPending(t *testing.T) {
	...
	if len(pending) != 6 {
		t.Errorf("expected 6 pending, got %d", len(pending))
	}
...
func TestRunner_Inspect_AllPending(t *testing.T) {
	...
	if len(status) != 6 {
		t.Fatalf("expected 6 statuses, got %d", len(status))
	}
...
func TestRunner_PendingCount_Some(t *testing.T) {
	...
	// 000001 is already applied, 000002-000006 are pending
	if count != 5 {
		t.Errorf("expected 5 pending, got %d", count)
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("unmet expectations: %v", err)
	}
...
func TestRunner_DryRun_Up_Pending(t *testing.T) {
	...
	if len(names) != 6 {
		t.Errorf("expected 6 migrations, got %d", len(names))
	}
...
func TestRunner_DryRun_NonePending(t *testing.T) {
	...
	if len(names) != 5 {
		t.Fatalf("expected 5 migrations, got %d", len(names))
	}
...
func TestRunner_UpgradeTo_AlreadyApplied(t *testing.T) {
	...
	if len(result.Skipped) != 6 {
		t.Errorf("expected 6 skipped, got %d", len(result.Skipped))
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("unmet expectations: %v", err)
	}
...
func TestRunner_PendingCount_NoMigrations(t *testing.T) {
	...
	// DirectionDown has 6 files (000001-000006).
...
func TestRunner_Summary_HasValues(t *testing.T) {
	...
	if s["pending"] != 5 {
		t.Errorf("expected pending=5, got %d", s["pending"])
	}
	if err := mock.ExpectationsWereMet(); err != nil {
		t.Errorf("unmet expectations: %v", err)
	}
